...
- Script owns the relationship between music and picture: main clock, visible singing, lyric/beat relationships, visual energy, diegetic sound, and allowed sync downgrades. It declares semantic segments, not guessed final timecodes.
...
After the complete song returns, inspect the same version's actual duration, waveform/audio facts, vocal activity, lyric timestamps, section structure, BPM/beat/energy changes, breaths, phrase endings, and sustained notes. Treat actual audio as truth; conflicting lyric timestamps are marked for review.
